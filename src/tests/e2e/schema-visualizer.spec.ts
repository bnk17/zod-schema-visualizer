...
test.describe('Page load', () => {
  ...
  test('shows the availability badge', async ({ page }) => {
    await expect(page.getByText(/available for a frontend role/i)).toBeVisible()
  })
...
test.describe('Preset selection', () => {
  ...
  test('auto-visualizes the form after selecting a preset', async ({ page }) => {
    await page.getByRole('button', { name: 'User Profile' }).click()
...
test.describe('Manual schema input', () => {
  ...
  test('Visualize button enables after typing a schema', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /visualize/i })).toBeEnabled()
  })
...
test.describe('Form validation', () => {
  ...
  test('"Back to form" returns to the form after success', async ({ page }) => {
    await page.getByRole('textbox', { name: /name/i }).fill('Alice')
    await page.getByRole('textbox', { name: /email/i }).fill('alice@example.com')
    await page.getByRole('spinbutton', { name: /age/i }).fill('25')
    await page.getByRole('button', { name: /submit/i }).click()
